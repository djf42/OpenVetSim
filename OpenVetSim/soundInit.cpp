...
addSoundToList(int type, int index, const char* name, int low_limit, int high_limit)
{
	...
	{
		snprintf(msgbuf, sizeof(msgbuf), "%s", "Too many tracks in sound list");
		log_message("", msgbuf);
		sts = -1;
	}
	...
	{
		snprintf(msgbuf, sizeof(msgbuf), "%s", "bad entry in sound list");
		log_message("", msgbuf);
		sts = -1;
	}
	return (sts);
...
initSoundList(void)
{
	...
	if (file == NULL)
	{
		...
		else
		{
			...
			snprintf(msgbuf, sizeof(msgbuf), "Failed to open %s %s", CSVfilename, errstr);
			log_message("", msgbuf);
		}
		//exit(-2);
	}
	...
	for (lines = 0; fgets(line, 1024, file); lines++)
	{
		...
		*out = 0;
		// Use sscanf with width specifiers instead of sscanf_s; widths are
		// sizeof(typeName)-1 = 7 and sizeof(sd.name)-1 = 31
		sts = sscanf(clean, "%7s %d %31s %d %d",
			typeName,
			&sd.index,
			sd.name,
			&sd.low_limit,
		...
		{
			snprintf(msgbuf, sizeof(msgbuf), "sscanf returns %d for line: \"%s\"\n", sts, line);
			log_message("", msgbuf);
		}
	}
	return (0);
