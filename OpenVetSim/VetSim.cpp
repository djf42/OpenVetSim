...
#ifdef _WIN32
// Serial-port RTS control (Windows only — used to drive an external pulse indicator)
HANDLE hComm = NULL;
int pulseState = 0;
...
#else
// POSIX stub — serial-port pulse indicator not available outside Windows
void setPulseState(int /*val*/) {}
#endif  // _WIN32
...
#else
	do_command_read("hostname",     simmgr_shm->server.name, sizeof(simmgr_shm->server.name) - 1);
#endif
	ptr = getETH0_IP();
...
start_scenario(void)
{
	...
	sprintf_s(sessionsPath, sizeof(sessionsPath), "%s" PATH_SEP "scenarios", localConfig.html_path);
	...
	if (simmgr_shm->status.scenario.record > 0)
	{
		...
		else
		{
			while ((fileCountAfter = getVideoFileCount()) == fileCountBefore)
			{
				...
				sim_sleep_ms(OBS_START_SLEEP_TIME);
			}
		}
	}
