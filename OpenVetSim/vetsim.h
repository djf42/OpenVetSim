...
// platform.h supplies all system headers and cross-platform shims.
// On Windows it pulls in the full Win32/WinSock2 stack.
// On macOS/Linux it provides POSIX equivalents and compatible type aliases.
#include "platform.h"
...
struct scenario
{
	...
	time_t startTime;
	struct tm	tmStart;
	uint64_t elapsed_msec_scenario;  // was ULONGLONG
	uint64_t elapsed_msec_scene;     // was ULONGLONG
};
...
struct cpr
{
	uint64_t last;		// msec time of last compression  (was ULONGLONG)
...
{
	uint64_t last;		// msec time of last shock  (was ULONGLONG)
...
struct server
{
	...
	uint64_t msec_time;				// msec timer  (was ULONGLONG)
	int dbg1;
	int dbg2;
	int dbg3;
...
struct logfile
{
	SIM_MUTEX sema;	// Cross-platform mutex lock (was HANDLE on Windows)
	int		active;
	int		lines_written;
	char	filename[FILENAME_SIZE];
...
int getKeys(void);
int64_t getDcode(void);				// was __int64
...
// clock_gettime is a native POSIX function on macOS/Linux.
// On Windows it is implemented in simutil.cpp.
#ifdef _WIN32
...
// obsData uses HWND/DWORD which are Windows-only (OBS integration)
#ifdef _WIN32
struct obsData
{
	HWND obsWnd;
